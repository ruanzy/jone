...
public class DynamicSQL2
{
	...
	static
	{
		...
		try
		{
			URL url = DynamicSQL2.class.getClassLoader().getResource("mappers");
			...
			for (File file : xmls)
			{
				...
				is = DynamicSQL2.class.getClassLoader().getResourceAsStream(path);
				if (is == null)
				{
				...
				for (Element e : list)
				{
					...
					// String innerXML = outerXML.replaceAll("^<" + nodeName +
					// ".*?>|</" + nodeName + ">$", "");
					// String sql =
					// e.asXML().getTextContent().replaceAll("\\s+|\t|\r|\n",
					// " ").trim();
					// System.out.println(outerXML);
					outerXML = outerXML.replaceAll("\n\t*", " ").trim();
					//System.out.println(outerXML);
					sqls.put(ns + "." + sqlid, outerXML);
				}
			}
		}
		catch (Exception e)
	...
	public static SQL getSQL3(String sqlid, Map<String, ?> params) throws Exception{
		...
		if (where != null)
		{
			...
			for (Element e : ifs)
			{
				...
				if(test){
					...
					//String text = parse(e.getText(), "#{", "}#", params);
					sb.append(" ");
					sb.append(text);
				}
			}
		...
		SQL tt = parse(sql.getText(), "#{", "}#", params);
		return tt;
	}
	...
	public static void main(String[] args) throws Exception
	{
		...
		SQL sql = DynamicSQL2.getSQL3(sqlid, params);
