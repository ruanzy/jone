...
//    		}
	...
	function buildPager(){
		...
		if(page== 1 && total == 0){
			...
			if(total == 0){
				...
				code.push('<table width=""><tr>');
				if(page > 1){
					code.push('<td style="text-align:center;border: 1px solid #e1e2e3;padding: 5px 10px;">');
					code.push('<a href="javascript:void(0)" class="pager-prev">', prev ,'</a>');	
					code.push('</td>');	
				}
				var begin = Math.max((page - 5), 1);
				var end = Math.min((page + 4), dataCache.length);
				for(var k = begin; k <= end; k++){
					code.push('<td style="text-align:center;border: 1px solid #e1e2e3;padding: 5px 10px;');
					if(k == page){
						code.push('background-color: #428bca;color: #fff;');
					}
					code.push('">');
					code.push('<a href="javascript:void(0)" class="pager-page"');	
					if(k == page){
						code.push('style="color: #fff;"');
					}
					code.push('">', k ,'</a>');	
					code.push('</td>');
				}
				if(dataCache.length != page || opts['hasNext']){
					code.push('<td style="text-align:center;border: 1px solid #e1e2e3;padding: 5px 10px;">');
					code.push('<a href="javascript:void(0)" class="pager-next">', next ,'</a>');	
					code.push('</td>');	
				}
				code.push('</tr></table>');
	...
	function renderPage(page){
		var grid = this;
		var opts = grid.data('options');
		var cacheData = dataCache[page - 1];
		if(!cacheData){
			var pagesize = opts['pagesize'];
			var bigData = opts['bigData'];
			var baseparams = {};
			baseparams[opts['pageName']] = page;
			baseparams[opts['pageSizeName']] = opts.pagesize;
			baseparams[opts['bigDataName']] = bigData;
			if(opts.condition){
			...
			if(opts.url){
				$.ajax({
					...
					success: function(result){	
						...
						if(opts.pager){	
							sessionStorage['QueryCacheKey'] = data['QueryCacheKey'];
			...
			if(opts.pager){	
				var total = cacheData.length; 
				//var rows = data['data']; 
				//var pc = Math.ceil(total/pagesize);
				grid.data('total', total);
				grid.data('page', page);
				renderData.call(grid, cacheData);
				buildPager.call(grid);
			}else{
				renderData.call(grid, cacheData);
			}
	...
	$.fn.biggrid.methods = {
		init: function(options) {
			var settings = $.extend({}, $.fn.biggrid.defaults, options);
