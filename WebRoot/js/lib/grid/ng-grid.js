			...
			if(opts.sm){
				if(opts.sm == 'm'){
					if(tr.hasClass('selected')){
						...
						var len = tbody.find('.sm div.unchecked').size();
						if(len == 0){
...
angular.module("grid", []).directive('grid', function($compile) {
	var directive = {};
	...
	// directive.template = '<div class="kk"></div>';
	directive.compile = function compile(element, attrs, transclude) {
		return function(scope, elem, attrs){
				//console.log(name + ': pre link');
				var opts = scope.config;
				opts.onRenderBefore = function(html){
					var pscope = scope.$parent;
					return $compile(html)(pscope);
				};
				var e = $(elem);
				var g = e.grid(opts);
				scope.config.getGrid = function(){
					return g;
				}
		};
	};
